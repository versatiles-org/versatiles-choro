...
import { resolveDataPath, resolveTempPath } from './filesystem';
...
describe('Filesystem Utilities', () => {
	...
	describe('resolve_data', () => {
		...
		it('resolves nested paths', () => {
			const result = resolveDataPath('subdir', 'file.json');
		...
		it('normalizes paths correctly', () => {
			const result = resolveDataPath('foo', '..', 'bar', 'file.txt');
		...
		it('handles empty path segments', () => {
			const result = resolveDataPath('', 'file.txt');

			expect(result).toContain('file.txt');
		});

		it('prevents path traversal with ../', () => {
			expect(() => resolveDataPath('../etc/passwd')).toThrow(PathTraversalError);
		});
		...
		it('handles leading slashes by stripping them', () => {
			const result = resolveDataPath('/subfolder/file.txt');
		...
		it('handles Windows-style paths', () => {
			const result = resolveDataPath('folder\\file.txt');
	...
	describe('resolve_temp', () => {
		...
		it('resolves nested paths in temp directory', () => {
			const result = resolveTempPath('session', 'data.tmp');
		...
		it('prevents path traversal in temp directory', () => {
			expect(() => resolveTempPath('../etc/passwd')).toThrow(PathTraversalError);
		});
		...
		it('handles multiple path segments', () => {
			const result = resolveTempPath('session-123', 'uploads', 'file.tmp');
		...
		it('normalizes temp paths', () => {
			const result = resolveTempPath('a', 'b', '..', 'c', 'file.tmp');
	...
	describe('path traversal edge cases', () => {
		it('handles null bytes in paths', () => {
			...
			const result = resolveDataPath('file.txt\0');
			expect(result).toBeDefined();
		});

		it('handles URL-encoded traversal attempts', () => {
			// %2e%2e%2f is ../ URL encoded
			const result = resolveDataPath('%2e%2e%2ftest.txt');
		...
		it('prevents double-encoded traversal', () => {
			expect(() => resolveDataPath('%252e%252e%252ftest.txt')).not.toThrow();
		});
		...
		it('handles UNC paths as regular paths', () => {
			...
			const result = resolveDataPath('\\\\server\\share\\file.txt');
			expect(result).toBeDefined();
		});
	});
	...
	describe('path resolution with environment variables', () => {
		...
		it('uses TEMP_PATH environment variable when set', () => {
			const result = resolveTempPath('test.tmp');
	...
	describe('security validations', () => {
		it('prevents accessing parent directory', () => {
			expect(() => resolveDataPath('..')).toThrow(PathTraversalError);
		});
		...
		it('allows safe relative paths within boundaries', () => {
			const result = resolveDataPath('safe', 'path', 'file.txt');
		...
		it('allows dot files within safe directory', () => {
			const result = resolveDataPath('.gitignore');

			expect(result).toContain('.gitignore');
		});

		it('allows hidden directories within safe directory', () => {
			const result = resolveDataPath('.config', 'settings.json');
	...
	describe('real-world attack scenarios', () => {
		...
		it('prevents /proc access', () => {
			expect(() => resolveDataPath('../../../proc/self/environ')).toThrow(PathTraversalError);
		});

		it('prevents Windows system file access', () => {
			expect(() => resolveDataPath('../../../windows/system32/config/sam')).toThrow(
				PathTraversalError
			);
		});
		...
		it('handles C: drive paths as regular paths on Unix', () => {
			...
			const result = resolveDataPath('C:\\Windows\\System32');
			expect(result).toBeDefined();
		});

		it('handles chained traversal attempts', () => {
			expect(() => resolveDataPath('a', '..', '..', 'b', '..', '..', 'etc', 'passwd')).toThrow(
				PathTraversalError
			);
		});
	});
